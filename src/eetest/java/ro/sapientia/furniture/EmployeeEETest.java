...
class EmployeeEETest {
    ...
    private List<Employee> employeeListWithMultipleEmployees = new ArrayList<Employee>(Arrays.asList(
            new Employee(1L, "Nagy", "Andor", 20, "Worker", null),
            new Employee(2L, "Kiss", "Erika", 22, "Worker", null),
            new Employee(3L, "Bartha", "Adam", 30, "Manager", null),
            new Employee(4L, "Feher", "Andrea", 20, "Worker", null)
    ));
    ...
    void testFindAllEmployeesWithOneEmployeeShouldSucceed() throws Exception {
        ...
        entityManager.persist(entityManager.merge(DatabaseMock.employeesWithOneEmployee.get(0)));
        mvc.perform(get("/employee/all").contentType(MediaType.APPLICATION_JSON))
                .andExpect(status().isOk())
                .andExpect(content().contentTypeCompatibleWith(MediaType.APPLICATION_JSON))
...
//    }
    ...
    public void testFindEmployeesByIdShouldSucceed() throws Exception {
        ...
        mvc.perform(get("/employee/find/{id}", 1L))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.first_name", is(DatabaseMock.employeesWithOneEmployee.get(0).getFirst_name())))
                .andExpect(jsonPath("$.last_name", is(DatabaseMock.employeesWithOneEmployee.get(0).getLast_name())))
                .andExpect(jsonPath("$.age", is(DatabaseMock.employeesWithOneEmployee.get(0).getAge())))
                .andExpect(jsonPath("$.employment_type", is(DatabaseMock.employeesWithOneEmployee.get(0).getEmployment_type())));
    }
    ...
    public void testCreateEmployeeShouldSucceed() throws Exception {
        ...
        mvc.perform(post("/employee/create")
                        ...
                        .content(objectMapper.writeValueAsString(DatabaseMock.employeesWithOneEmployee.get(0))))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.first_name", is(DatabaseMock.employeesWithOneEmployee.get(0).getFirst_name())))
                .andExpect(jsonPath("$.last_name", is(DatabaseMock.employeesWithOneEmployee.get(0).getLast_name())))
                .andExpect(jsonPath("$.age", is(DatabaseMock.employeesWithOneEmployee.get(0).getAge())))
